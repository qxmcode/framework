...
class StartServer extends Command
{
    ...
    {
        $this
            ->setDescription('Start Qxmcode servers.')
            ->addOption('daemonize', 'd', InputOption::VALUE_OPTIONAL, 'Qxmcode server daemonize', false)
    ...
    private function startServer()
    {
        ...
        if ($this->daemonize) {
            ...
            $this->io->success('Qxmcode server start success.');
        }
    ...
    private function stopServer()
    {
        ...
        if ($pid && Process::kill($pid, SIG_DFL)) {
            if (!Process::kill($pid, SIGTERM)) {
                $this->io->error('old Qxmcode server stop error.');
                die();
            }
